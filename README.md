...
- 支持多设备策略按**序号**精确选更：`CF_PROFILE_ID` 逗号分隔登记策略，`CF_PROFILE_INDEX` 指定本次更新哪些序号
...
|`CF_PROFILE_INDEX`|本次要更新的策略**序号**，逗号分隔（如 `1,3`）；留空=不更新任何自定义策略。不支持 `0`（默认策略无法批量更新）|❌可选  |
...
1. **登记策略清单**：把所有自定义设备策略 ID 填入 `CF_PROFILE_ID`，逗号分隔，脚本自动编号，序号从 **1** 开始依次对应：
   ...
   > 序号 `[0]` 固定代表账户默认设备策略，**仅可切换、无法批量更新**，因此 `CF_PROFILE_INDEX` 不支持填 `0`。
...
2. **指定本次要更新的序号**：在 `CF_PROFILE_INDEX` 填写序号，逗号分隔：
   ...
   > 脚本会对序号做去重、排序与范围校验（超出 `1 ~ N` 或填 `0` 会报错终止）。
...
CF_PROFILE_INDEX=1                        # 本次更新的序号，逗号分隔；空=不更新
...
正常输出示例（`CF_PROFILE_INDEX=1`，仅更新序号 1）：

```
ℹ️ 策略索引对照表：
   [0] 账号默认设备策略（仅可切换，无法批量更新）
   [1] 邮箱ID
   [2] 安卓ID
   [3] 苹果ID
   [4] 电脑ID
...
**Q：报错 `更新序号N超出范围！可用更新序号：1 ~ M`？**  
A：`CF_PROFILE_INDEX` 填写的序号超出了 `CF_PROFILE_ID` 登记的策略数量。请对照运行时打印的「策略索引对照表」，只填 `1 ~ M` 范围内的序号。

**Q：报错 `CF_PROFILE_INDEX 不支持填写0`？**  
A：序号 `[0]` 是账户默认设备策略，Cloudflare 不支持对其批量更新分流规则。请将默认策略之外的自定义策略登记到 `CF_PROFILE_ID` 后，用序号 `1` 起选更。
